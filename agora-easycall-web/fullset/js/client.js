$(document).ready(function(){
  ...
  function displayInfo(stream){
    var $p = $('<p id="infoStream' + stream.getId() + '">' + stream.getId() + '进入会议室</p>');
    $(".info").append($p);
  ...
  client.on('peer-leave', function(evt){
    var $p = $('<p id="infoStream' + evt.uid + '">' + evt.uid + '退出了会议室</p>');
    $(".info").append($p);
    setTimeout(function(){$p.remove();}, 10*1000);
  ...
  client.on('stream-removed', function(evt){
    var stream = evt.stream;
    var $p = $('<p id="infoStream' + stream.getId() + '">' + stream.getId() + '退出了会议室</p>');
    $(".info").append($p);
    setTimeout(function(){$p.remove();}, 10*1000);

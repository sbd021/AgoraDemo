...
enum ERROR_CODE_TYPE
{
    ...
    ERR_ADM_RECORD_AUDIO_SILENCE = 1019,
    ERR_ADM_PLAYOUT_ABNORMAL_FREQUENCY = 1020,
    ERR_ADM_RECORD_ABNORMAL_FREQUENCY = 1021,
};
...
public:
	...
	virtual void onSetParameterDone(const char* key, int result) {}
...
public:
	...
	virtual int renewDynamicKey(const char* key) = 0;
};
...
public:
    ...
    int setLogFilter(unsigned int filter) {
		return m_parameter->setUInt("rtc.log_filter", filter);
	}
    int enableAudioQualityIndication(bool enabled) {
        return m_parameter->setBool("rtc.audio_quality_indication", enabled);
    }
...
protected:
	...
	int setProfile(const char* format, ...) {
		...
		vsnprintf(buf, sizeof(buf)-1, format, args);
		va_end(args);
		return m_parameter->setProfile(buf, true);
	}

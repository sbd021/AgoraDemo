...
- (void)processTouches:(NSSet *)touches
{
    ...
    int newRating = (int) (point.x / (width + margin)) + 1;
    if (newRating > 5)
        return;
...
- (void)layoutSubviews
{
    ...
    margin = (CGRectGetWidth(self.frame) - 5 * CGRectGetWidth(_s1.frame)) / 4;
    
    [_s1 setFrame:CGRectMake(0,                  0, width, height)];
    [_s2 setFrame:CGRectMake(width + margin,     0, width, height)];
    [_s3 setFrame:CGRectMake(2 * (width + margin), 0, width, height)];
    [_s4 setFrame:CGRectMake(3 * (width + margin), 0, width, height)];
    [_s5 setFrame:CGRectMake(4 * (width + margin), 0, width, height)];
}
